...
function step(state) {
    if (state.steps > 0) {
        ...
        stepsSinceCrossing[state.cableCode]++;
        ...
        if (!theMatrix[state.x][state.y] || theMatrix[state.x][state.y].marking === state.cableCode) {
            ...
            theMatrix[state.x][state.y][state.cableCode] = stepsSinceCrossing[state.cableCode];
        } else {
